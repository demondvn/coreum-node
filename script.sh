...
# Install GO
#  rm -rf /usr/local/go
# curl -Ls https://go.dev/dl/go1.19.5.linux-amd64.tar.gz |  tar -xzf - -C /usr/local
# eval $(echo 'export PATH=$PATH:/usr/local/go/bin' |  tee /etc/profile.d/golang.sh)
...
ln -s $HOME/$FOLDER/$CHAIN/$COSMOVISOR/genesis $HOME/$FOLDER/$CHAIN/$COSMOVISOR/current
 ln -s $HOME/$FOLDER/$CHAIN/$COSMOVISOR/current/bin/$BINARY /usr/bin/$BINARY
